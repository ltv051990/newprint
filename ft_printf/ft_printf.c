...
int ft_read_nbsize_flags(char *format, t_struct *flags)
{
	...
	if ((a = 1) && *format == 'z')
		flags->mod_size = 'z';
	else if ((a = 1) && *format == 'j' && flags->mod_size != 'z')
		flags->mod_size = 'j';
	else if ((a = 2) && *format == 'l' && *(format + 1) == 'l' && \
		flags->mod_size != 'j' && flags->mod_size != 'z')
		flags->mod_size = 'L';
	else if ((a = 1) && *format == 'l' && flags->mod_size != 'L' && \
		flags->mod_size != 'j' && flags->mod_size != 'z')
		flags->mod_size = 'l';
	else if ((a = 1) && *format == 'h' && *(format + 1) != 'h' &&\
		flags->mod_size != 'l' && flags->mod_size != 'L' \
		&& flags->mod_size != 'j' && flags->mod_size != 'z')
		flags->mod_size = 'h';
	else if ((a = 2) && *format == 'h' && *(format + 1) == 'h' &&\
...
int		ft_read_spec(char *format, va_list ap, t_struct *flags)
{
	...
	{	
		...
		if (format[index] == '.' && !(ft_isdigit(format[index + 1])) && ++index) // flags->prezition = 0;
			flags->presizion = 0;
		while (ft_isdigit(format[index]))
			index++;
